...
class RemoveRoutePathButton extends React.Component<IRemoveRoutePathButtonProps> {
  private removeRoutePath = async () => {
    ...
    const routePathsAfterRemove = this.props.routePathStore!.existingRoutePaths.filter(
      (rp) => {
        const isRpToRemove =
  ...
  render() {
    ...
    return (
      <SaveButton
        className={(s.removeButton, !isRoutePathDeletable ? s.disabledHoverButton : undefined)}
