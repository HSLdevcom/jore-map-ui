...
@observer
class RoutePathListTab extends React.Component<
  IRoutePathListTabProps,
  IRoutePathListTabState
> {
  ...
  private getReadOnlyGroupedRoutePaths = (routePaths: IRoutePath[]): IRoutePath[][] => {
    ...
    list.sort(
      (a: IRoutePath[], b: IRoutePath[]) => b[0].startDate.getTime() - a[0].startDate.getTime()
    )
  ...
  private fetchStopNames = async () => {
    ...
    if (this.props.isEditing) {
      ...
      routePaths = this.props.routePathMassEditStore!.massEditRoutePaths!.map((massEditRp) => {
        if (massEditRp.isNew) {
  ...
  private renderBottomBarButtons = () => {
    ...
    const isCopyRoutePathButtonDisabled =
      isEditing && this.props.routeId !== this.props.routeListStore!.routeIdToEdit
    return (
  ...
  private openCopyRoutePathView = (isEditing: boolean) => () => {
    ...
    if (!isEditing) {
      this.props.routePathMassEditStore!.init({
        routeId,
        routePaths: this.props.originalRoutePaths,
      })
  ...
  private selectRoutePathToBeCompared = (routePath: IRoutePath) => {
    const isTheSameRoutePathCurrentlySelected =
      ...
      RoutePathUtils.getAreRoutePathsEqual(this.state.routePathSelectedToBeCompared, routePath)
    if (
      ...
      this._setState({
        routePathSelectedToBeCompared: isTheSameRoutePathCurrentlySelected ? null : routePath,
      })
    }
  }
    ...
    return (
      <div className={s.routePathListTab}>
        ...
        {this.state.hasOldRoutePaths && (
          ...
          >
            {!this.props.areAllRoutePathsVisible && <div className={s.threeDots}>...</div>}
