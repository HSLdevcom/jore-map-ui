...
class CopyRoutePathView extends React.Component<ICopyRoutePathViewProps, ICopyRoutePathState> {
  ...
  private toggleRoutePath = (routePath: IRoutePath) => async () => {
    ...
    if (shouldRemove) {
      ...
      const { routeId, startDate, direction } = routePath
      ...
      const isFoundFromQueryResult = this.state.routePathQueryResults.find((rpQueryResult) => {
      ...
      <div className={s.copyRoutePathView} data-cy="copyRoutePathView">
        ...
        >
          <div>
            ...
            <span className={TransitTypeUtils.getColorClass(transitType)}>{lineId}</span>{' '}
            reitille{' '}
            <span className={TransitTypeUtils.getColorClass(transitType)}>{routeId}</span>
          </div>
        </SidebarHeader>
          ...
          {routePathsToCopy.length === 0 ? (
            ...
            <table className={s.routePathTable}>
              <tbody className={s.routePathTableHeader}>
                ...
                {routePathsToCopy.map((rpToCopy: IRoutePathToCopy, index: number) => {
                    ...
                    <tr key={`rpToCopyRow-${index}`} data-cy={`rpToCopyRow-${index}`}>
                      <td>
                        <Dropdown
                          ...
                          onChange={this.onDirectionChange(rpToCopy.id)}
                        />
                      </td>
                      <td className={s.maxWidthColumn}>{rpToCopy.routePath.originFi}</td>
                      <td className={s.maxWidthColumn}>{rpToCopy.routePath.destinationFi}</td>
                      <td className={s.maxWidthColumn}>
                        {Moment(rpToCopy.routePath.startDate).format('DD.MM.YYYY')}
                      </td>
                      <td className={s.maxWidthColumn}>
                        {Moment(rpToCopy.routePath.endDate).format('DD.MM.YYYY')}
                      </td>
                      <td>
                        <Button
                          className={s.removeRoutePathToCopyButton}
                          hasReverseColor={true}
                          onClick={this.removeRoutePathToCopy(rpToCopy.id)}
                        >
                          <FaTrashAlt />
                        </Button>
                      </td>
              ...
              <div className={classnames(s.flexRow, s.routePathSelectView)}>
                ...
                {this.state.routePathQueryResults.length === 0 ? (
                  <div className={s.noQueryResults}>
                    Reitiltä {this.state.selectedRouteId} ei löytynyt reitinsuuntia.
                  </div>
                ) : (
                  <>
                              ...
                              return (
                                <tr
                                  ...
                                  className={classnames(
                                    s.tableRow,
                                    isSelected ? s.selectedRow : undefined,
                                    isValid ? s.validRow : undefined
                                  )}
                                  onClick={this.toggleRoutePath(routePath)}
                                  data-cy={`rpQueryResult`}
                                >
                                  <td>{routePath.direction}</td>
                                  <td className={s.maxWidthColumn}>{routePath.originFi}</td>
                                  <td className={s.maxWidthColumn}>
                                  ...
                                  <td>
                                    <div
                                      className={s.circle}
                                      style={
                                        isSelected ? selectedBackgroundColorStyle : undefined
                                      }
