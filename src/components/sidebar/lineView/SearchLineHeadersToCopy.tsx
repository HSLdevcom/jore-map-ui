  ...
  observer((props: ISearchLineHeadersToCopyProps) => {
    ...
    useEffect(() => {
      const fetchLineHeaders = async () => {
        ...
        setLineHeaders(
          lineHeaders.sort((a: ILineHeader, b: ILineHeader) =>
            ...
            {props.lineHeaders.length > 0 ? (
              <div className={s.lineHeaderTableContainer}>
                <table className={s.lineHeaderTable}>
                  <tbody>
                    <tr>
                      <th className={classnames(s.inputLabel, s.columnHeader)}>LINJAN NIMI</th>
                      <th className={classnames(s.inputLabel, s.columnHeader)}>VOIM. AST</th>
                      <th className={classnames(s.inputLabel, s.columnHeader)}>VIIM. VOIM.</th>
                      <th />
                      <th />
                      <th />
                    </tr>
                    {props.lineHeaders.map((lineHeader: ILineHeader, index: number) => {
                      return (
                        <tr
                          className={classnames(
                            s.tableRow,
                            selectedRowIndexes.includes(index) ? s.selectedRow : undefined
                          )}
                          key={`lineHeaderRow-${index}`}
                          data-cy="lineHeaderRow"
                          onClick={() => toggleSelectedRow({ index })}
                        >
                          <td className={s.tableCell}>{lineHeader.lineNameFi}</td>
                          <td className={s.tableCell}>{toDateString(lineHeader.startDate)}</td>
                          <td className={s.tableCell}>{toDateString(lineHeader.endDate)}</td>
                        </tr>
                      )
                    })}
                  </tbody>
