...
@observer
class RoutePathNeighborLinkLayer extends Component<
      ...
      <>
        {clusteredNeighborLinkMapEntries.map(([bounds, neighborLinkCluster], index) => {
          if (neighborLinkCluster.length === 1) {
            ...
            const nodeToRender =
              this.props.routePathLayerStore!.neighborToAddType === NeighborToAddType.AfterNode
          ...
          if (neighborLinkCluster.length > 1) {
            const searchNodes = neighborLinkCluster.map((neighborLink: INeighborLink) => {
              const nodeToRender =
                this.props.routePathLayerStore!.neighborToAddType ===
            ...
            const popupData: ISelectRoutePathNeighborPopupData = {
              neighborNodes: neighborLinkCluster.map((neighborLink: INeighborLink) => {
                const nodeToRender =
                  this.props.routePathLayerStore!.neighborToAddType ===
                  NeighborToAddType.AfterNode
                    ? neighborLink.routePathLink.endNode
