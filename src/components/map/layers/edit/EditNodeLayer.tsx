...
const EditNodeLayer = inject(
  ...
  observer((props: IEditNodeLayerProps) => {
    ...
    const updateLinkGeometry = useCallback(
      (leafletId: number) => {
        const editableLink = editableLinks.find((link: any) => link._leaflet_id === leafletId)
        if (editableLink) {
    ...
    const renderNodes = () => {
      ...
      return (
        <>
          ...
          <DashedLine startPoint={node.coordinates} endPoint={node.coordinatesProjection} />
        </>
      )
    }
    ...
    const renderNode = ({
      ...
      return (
        <NodeMarker
          ...
          shortId={nodeLocationType === 'coordinates' ? NodeUtils.getShortId(node) : undefined}
          hastusId={
            nodeLocationType === 'coordinates' && node.stop ? node.stop.hastusId : undefined
          }
